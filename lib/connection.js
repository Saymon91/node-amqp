...
var { methods, classes } = definitions;
...
Connection.prototype.exchange = function (name, options, callback) {
  if (callback instanceof Function) {
    return this.createExchange(name, options, (err, exchange) => {
      if (err) {
        return callback(err);
      }
  ...
  const exchange = this.createExchange(name, options);
...
Connection.prototype.checkExchange = function (exchangeName, callback) {
  let exchange = this.exchanges[exchangeName];
  if (exchange && exchange.state === 'open') {
    return callback instanceof Function && callback(null, true);
  }

  this.addExchange(this.createExchange(exchangeName, { passive: true }, (err, exchange) => {
    if (err) {
      return callback(null, false);
    }

    exchange.close(null, () => callback(null, true));
  }));
};

Connection.prototype.createExchange = function (name, options, callback) {
  let exchange = this.exchanges[name];
  if (exchange && exchange.state === 'open') {
    callback instanceof Function && callback(null, exchange);
    return exchange;
  }
  ...
  try {
    ...
    callback instanceof Function && callback(exception);
    return exception;
  }

  exchange = new Exchange(this, channel, name, options, callback);
  return exchange;
};

Connection.prototype.addExchange = function (exchange) {
  this.channels[exchange.channel] = exchange;
...
Connection.prototype.exchangeClosed = function (name) {
  ...
  if (exchange) {
    ...
    delete this.channels[exchange.channel];
  }
};
...
Connection.prototype.queue = function (name, options, callback) {
  if (callback instanceof Function) {
    return this.createQueue(name, options, (err, queue, ...args) => {
      if (err) {
        return callback(err);
      }

      this.addQueue(queue);
      callback(null, queue, ...args);
    });
  }
...
Connection.prototype.createQueue = function (name, options, callback) {
  let queue = this.queues[name];
  if (queue && queue.state === 'open') {
    callback instanceof Function && callback(null, queue);
    return queue;
  }
  ...
  queue = new Queue(this, channel, name, options, callback);
  return queue;
};

Connection.prototype.addQueue = function (queue) {
  if (queue instanceof Error) {
    return;
  }
  this.queues[queue.name] = queue;
  this.channels[queue.channel] = queue;
};

Connection.prototype.checkQueue = function (queueName, callback) {
  let queue = this.queues[queueName];
  if (queue && queue.state === 'open') {
    return callback instanceof Function && callback(null, true);
  }
  
  this.addQueue(this.createQueue(queueName, { passive: true }, (err, queue) => {
    if (err) {
      return callback(null, false);
    }
    
    queue.close(null, () => callback(null, true));
  }));
};
...
Connection.prototype.queueClosed = function (name) {
  ...
  if (queue) {
    ...
    delete this.channels[queue.channel];
  }
};
...
Connection.prototype._onMethod = function (channel, method, args) {
  //console.error('CONNECTION', method.name, args);
...
Connection.prototype.generateChannelId = function () {
  var channelId = 1;
  while (this.channels[channelId] && channelId !== channelMax) {
    channelId++;
  }
  ...
  this.channelCounter = Object.keys(this.channels).length;
  return channelId;
};
