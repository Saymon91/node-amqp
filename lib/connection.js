...
Connection.prototype._sendMethod = function (channel, method, args) {
  debug && debug(channel + " < " + method.name + " " + JSON.stringify(args));
  console.log(channel, JSON.stringify(method, true, 2), JSON.stringify(args, true, 2));

  var sendBuffer = this._sendBuffer;
  sendBuffer.used = 0;
  sendBuffer[sendBuffer.used++] = constants.FrameType.METHOD;
  serializer.serializeInt(sendBuffer, 2, channel);

  var lengthIndex = sendBuffer.used;

  serializer.serializeInt(sendBuffer, 4, 42); // replace with actual length.
  var startIndex = sendBuffer.used;

  serializer.serializeInt(sendBuffer, 2, method.classIndex); // short, classId
  serializer.serializeInt(sendBuffer, 2, method.methodIndex); // short, methodId

  serializer.serializeFields(sendBuffer, method.fields, args, true);

  var endIndex = sendBuffer.used;

  // write in the frame length now that we know it.
  sendBuffer.used = lengthIndex;
  serializer.serializeInt(sendBuffer, 4, endIndex - startIndex);
  sendBuffer.used = endIndex;

  sendBuffer[sendBuffer.used++] = 206; // constants.frameEnd;

  var c = new Buffer(sendBuffer.used);
  sendBuffer.copy(c);
