...
Queue.prototype.basicConsumeOn = function () {
...
Queue.prototype.subscribe = function (options, messageListener) {
  ...
  return this.subscribeRaw(rawOptions, function (message) {
    var contentType = message.contentType;
    var decoder = new StringDecoder('utf8');

    if (contentType === null && message.headers && message.headers.properties) {
      contentType = message.headers.properties.content_type;
    }
    ...
    message.addListener('end', function () {
      ...
      for (var i = 0, l = msgProperties.length; i<l; i++) {
        if (message[msgProperties[i].name]) {
          deliveryInfo[msgProperties[i].name] = message[msgProperties[i].name];
        }
      }
      ...
      if (messageListener) messageListener(json, headers, deliveryInfo, message);
      self.emit('message', json, headers, deliveryInfo, message);
    });
  });
};
...
Queue.prototype.bind = function (exchange, routingKey, args, callback) {
  var self = this;
...
Queue.prototype.unbind = function (exchange, routingKey, callback) {
  var self = this;
  ...
  if (_.isFunction(callback)) this._unbindCallback = callback;
  var exchangeName = exchange instanceof Exchange ? exchange.name : exchange;
...
Queue.prototype.bind_headers = function (/* [exchange,] matchingPairs[, callback] */) {
  var self = this;
  ...
  var exchange, matchingPairs, callback;
...
Queue.prototype.unbind_headers = function (/* [exchange,] matchingPairs[, callback] */) {
  var self = this;
...
Queue.prototype.channelOpenOk = function (channel, args) {
...
Queue.prototype.queueDeclareOk = function (channel, args) {
  this.state = 'open';
...
Queue.prototype.basicConsumeOk = function (channel, args) {
  debug && debug('basicConsumeOk', util.inspect(args, null));
};

Queue.prototype.queueBindOk = function () {
...
Queue.prototype.confirmSelectOk = function () {
  this._sequence = 1;
  this.confirm = true;
};

Queue.prototype.channelClose = function (channel, args) {
  this.state = "closed";
  this.closeOK();
...
Queue.prototype.channelCloseOk = function () {
  this.connection.queueClosed(this.name);
  this.emit('close');
};

Queue.prototype.basicDeliver = function (channel, args) {
  this.currentMessage = new Message(this, args);
};

Queue.prototype.basicClose = function (channel, args) {
...
Queue.prototype._onMethod = function (channel, method, args) {
  console.log(method, JSON.stringify(args, true, 2));
  ...
  var methodHandler = this[method.name];

  if (typeof methodHandler === 'function') {
    methodHandler.call(this, channel, args);
  }
